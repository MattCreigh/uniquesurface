...
The state directory at ``~/.local/state/trinity/templates/`` is the
...
hashes match what was on disk before trinity touched anything.

This module is intentionally not auto-invoked; the orchestrator calls
it from ``trinity install`` and ``trinity qml-update-templates``.
"""
...
from trinity.manifest import sha256_bytes
...
def extract(targets: Iterable[tuple[str, Path]] | None = None) -> list[Path]:
    ...
    Sentinel markers from a previous trinity patch are stripped before
    ...
    """
    from trinity.theme.drift import strip_sentinels
    ...
    for name, src in targets or DEFAULT_TARGETS:
        ...
        atomic_write_bytes(dest, clean.encode("utf-8"), mode=0o644)
        out.append(dest)
    return out
